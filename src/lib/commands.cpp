...
std::string git_add_file_helper(std::string old_tree_hash, typename fs::path::iterator file_it,
        ...
        if (node.name == *file_it) {
            ...
            // SubCase 2 : file_it still refers to a folder
                ...
                std::string tree_hash =
                    git_add_file_helper(node.hash, file_it, end_it, git_path, file_path);
...
std::string git_add_folder_helper(std::string old_tree_hash, typename fs::path::iterator file_it,
    ...
    for (auto node : old_tree_obj->directory) {
        ...
        if (node.name == *file_it) {
            ...
            // SubCase 2 : still haven't reached folder
                ...
                std::string tree_hash =
                    git_add_file_helper(node.hash, file_it, end_it, git_path, folder_path);
                // Q: how do I know that this is guaranteed to be a tree?
                tree_obj.add_entry("tree", node.name, tree_hash);
            }
        }
