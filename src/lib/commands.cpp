...
std::string git_add_folder_helper(std::string old_tree_hash,typename fs::path::iterator file_it, typename fs::path::iterator end_it, const fs::path git_path,const fs::path folder_path){
    // New Tree Object we are creating
    GitTree tree_obj(git_path);
    // Old Tree Object we are iterating over
    GitTree* old_tree_obj = dynamic_cast<GitTree*>(readObject(git_path,old_tree_hash));
    ...
    for (auto node:old_tree_obj->directory){
        ...
        if(node.name==*file_it){
            ...
            if (++check_it == end_it){
                ...
                std::string subtree_hash = writeTreeFromFolder(folder_path,false);
                // add to new_tree
            ...
            // SubCase 2 : still haven't reached folder
                ...
                std::string tree_hash = git_add_file_helper(node.hash,file_it,end_it,git_path,folder_path);
                // Q: how do I know that this is guaranteed to be a tree?
                tree_obj.add_entry("tree",node.name,tree_hash);
            }
        }
...
std::string git_add_folder(const fs::path folder_path){
    ...
    std::string old_tree_hash = getTreeHashOfHead(git_path);
    std::cout << "HashOfHead:" << old_tree_hash << std::endl;
    std::string new_tree_hash = git_add_folder_helper(old_tree_hash,file_it,folder_path.end(),git_path,folder_path);
...
std::string writeTreeFromFolder(const fs::path& adding_directory, bool index){
    ...
    for (auto entry: fs::directory_iterator(adding_directory)){
        ...
        if (fs::is_regular_file(entry)){
            ...
            std::string tree_hash = writeTreeFromFolder(path);
